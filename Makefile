...
# Makefile for re-worker-fileops
#
...
NAME := re-worker-fileops
SHORTNAME := replugin
TESTPACKAGE := replugin

RPMSPECDIR := ./contrib/rpm/
RPMSPEC := $(RPMSPECDIR)/re-worker-fileops.spec
...
srpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-fileops*src.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
...
rpm: rpmcommon
	...
	@find rpm-build -maxdepth 2 -name 're-worker-fileops*.rpm' | awk '{print "    " $$1}'
	@echo "#############################################"
